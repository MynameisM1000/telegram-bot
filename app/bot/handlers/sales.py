...
def parse_sales_report(text: str, branches: dict):
    ...
    for field in single_fields:
        match = re.search(rf'{field}.*?([0-9\s\.]+)', text_lower)
        if match:
    ...
    for i, line in enumerate(lines):
        ...
        for block in blocks:
            if line.startswith(block[:5]): 
                count_match = re.search(r'([0-9\s\.]+)', line.replace(block[:5], ''))
                count = 0
                if count_match:
                ...
                for j in range(1, 4):
                    if i + j < len(lines):
                        ...
                        if "сумма" in next_line:
                            ...
                            val_match = re.search(r'([0-9\s\.]+)', clean_line)
                            if val_match:
    ...
    # 4. Общая сумма
    total_match = re.search(r'общ[аяе].*?([0-9\s\.]+)', text_lower.replace('\n', ' '))
    if total_match:
